...
(function ($) {
  ...
  try {
    if ($("#smooth-wrapper").length && $("#smooth-content").length) {
      ...
      praveenSmoother = ScrollSmoother.create({
        smoothTouch: 0.2,
        smooth: 4,
        effects: true,
    ...
    // If the premium ScrollSmoother plugin fails to init for any reason,
    // fall back to native smooth scrolling instead of breaking every
    // other script that runs after this one.
    praveenSmoother = null;
    console.warn("ScrollSmoother failed to initialize, falling back to native scroll.", e);
  }

  function smoothSctoll() {
    // Delegated on document so this keeps working even for links that get
    // added to the DOM later (e.g. the offcanvas mobile menu, which main.js
    // clones in from the header nav AFTER this script runs).
    $(document).on("click", 'a[href^="#"]', function (event) {
      ...
      if (praveenSmoother) {
        // Use GSAP ScrollSmoother's own scrollTo so it matches the
        // transformed/smoothed scroll container instead of the native window.
        try {
          praveenSmoother.scrollTo(target[0], true, "top top");
        } catch (e) {
          $("html, body")
            .stop()
            .animate(
      ...
      // Close the mobile offcanvas menu if it's open
      if ($("body").hasClass("tw-offcanvas-2-open")) {
        $(".tw-offcanvas-2-close-btn").trigger("click");
      }
    });
  }
  smoothSctoll();
